...
if (!LLM_MODEL_NAME) {
  throw new Error(
    "Please set the MODEL_NAME environment variable in your .env file"
  );
}
  ...
  while (true) {
    ...
    const result = streamText({
      ...
      tools: {
        date: tool({
          description:
            "Use this tool when the user asks about the curret date. Get the current date and time as a string",
        ...
        weather: tool({
          ...
          execute: async (input) => {
            ...
            const response = await fetch(
              `http://goweather.xyz/weather/${location}`
            );
